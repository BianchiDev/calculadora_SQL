...
código);
 ...
 CALL MostrarProdutosQuartos(1);
 CALL MostrarProdutosQuartos(2);
 CALL MostrarProdutosQuartos(3);
 CALL MostrarProdutosQuartos(4);
 ...
 DELIMITER $$
 CREATE PROCEDURE atualizarRendaCliente (renda FLOAT,
codigo_cliente INT)
 BEGIN
...
DELIMITER $$
 CREATE PROCEDURE inserirEndereco (rua VARCHAR (100), numero INT,
bairro VARCHAR (100), cidade VARCHAR (100), estado VARCHAR
(100))
 BEGIN
...
DELIMITER $$
CREATE PROCEDURE MostrarConsumoCliente (codigo_cli INT, OUT total
DOUBLE)
BEGIN
SELECT
...
delimiter $$
create procedure listaProdutoProQuant (quant int)
begin
select * from produto where quantidade_prod > quant;
end;
$$ delimiter ;

call listaProdutoProQuant (3);
#2. Crie um procedimento com passagem de parâmetros de entrada para cadastrar um novo Produto;
delimiter $$
create procedure inserirProdutotos (nome varchar(100),qunat float, valor float, validade date)
begin
insert into produto values (null, nome, qunat, valor, validade);
select'Produto cadoastrado com sucesso!' as confirmação;
end;
$$ delimiter ;
call  inserirProdutotos('feijão', 50, 8.50, '2020-01-30');
call  inserirProdutotos('chiclete Big Big', @resultado,0.90, '2020-04-30');
select * from produto;
#3. Crie um procedimento que retorne por uma variável de saída o número total de clientes cadastrados no sistema do hotel;
delimiter $$
create procedure retornarQuantCliente(out total int)
begin
select count(cod_cli)into total from cliente;
end;
$$ delimiter ;
#variável blobal 
call retornarQuantCliente(@resultado);
select @resultado ;
...
delimiter $$
create procedure attProdutosPorcentagem(x float)
begin
update produto set valor_prod = valor_prod +(valor_prod * (x/100));
select 'Produtos atualizados com sucesso!' as confirmacao;
end;
$$ delimiter ;

call attProdutosPorcentagem (50);
...
delimiter $$
create procedure AtualizarValorTotalHosp (codigo int)
begin
update hospedagem set  valorTotal_hosp = valorconsumo_hosp + valorDiárias_hosp where cod_hosp = codigo;
select 'Hospedagens atualizadas com sucesso'as confirmação;
end;
$$ delimiter ;

call AtualizarValorTotalHosp (2) ;

#Crie um procedimento que se comporte como uma calculadora. Você deverá receber 02 números como parâmetros de entrada. Também deverá receber um parâmetro de entrada que informe o tipo da operação (+, -, /, *);
...
if (sinal = '+') then
	...
	if (sinal = '-') then
		...
		if (sinal = '*') then
			...
			select 'Digite um sinal válido : + ou - ouy * ou /' as Alerta;
            end if;
		end if;
	end if;
end if;
